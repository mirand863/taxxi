...
# Split level  True level  False level  T levels  F levels
# -----------  ----------  -----------  --------  --------
#           r          (-)           d       (-)  gfocpd
#           d           d            p         d  gfocp
#           p           p            c        pd  gfoc
...
def GetPredsFromPredStr(PredStr):
	Preds = PredStr.split(',')
	for Pred in Preds:
		if len(Pred) < 2 or Pred[1] != ':':
			die.Die("Invalid pred str '%s'" % PredStr)
	return Preds
...
def GetNameFromDict(Dict, Rank, Default = None):
	try:
		Name = Dict[Rank]
	except:
		Name = Default
...
def GetLDRNames(Tax1, Tax2):
	...
	for i in range(0, n):
		...
		if Name1 != None and Name1 != Name2:
			return ParentName, Name1, Name2
		elif Name1 != None and Name1 == Name2:
			ParentName = Name1
	return None, None, None

def GetLDRNamesFromLabels(Label1, Label2):
	Tax1 = GetTaxFromLabel(Label1)
	Tax2 = GetTaxFromLabel(Label2)
	return GetLDRNames(Tax1, Tax2)
